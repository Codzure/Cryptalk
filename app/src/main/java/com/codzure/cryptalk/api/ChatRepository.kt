...
class ChatRepository(
    private val chatApiService: ApiService,
    ...
    suspend fun getConversations(): Result<List<ConversationResponse>> =
        withContext(Dispatchers.IO) {
            try {
                val currentUser =
                    authRepository.getCurrentUser() ?: return@withContext Result.failure(
     ...
     */
    suspend fun createConversation(otherUserId: String): Result<ConversationResponse> =
        withContext(Dispatchers.IO) {
            try {
                val currentUser =
                    authRepository.getCurrentUser() ?: return@withContext Result.failure(
